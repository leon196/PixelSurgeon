...
Main.prototype = $extend(flash.display.Sprite.prototype,{
	...
	,onEnterFrame: function(event) {
		if(this.mouseClic) {
			var x = Math.floor(this.background.get_mouseX());
			var y = Math.floor(this.background.get_mouseY());
			...
			this.pixelsBackground.setPixel32(x,y - 1,this.brushColor);
			this.pixelsBackground.setPixel32(x - 1,y,this.brushColor);
			this.pixelsBackground.setPixel32(x,y + 1,this.brushColor);
			this.pixelsBackground.setPixel32(x + 1,y,this.brushColor);
		}
		if(this.movingUp) {
			if(this.velocityY >= 0.0) this.velocityY = -1.0;
			this.velocityY = Math.max(-200.,this.velocityY * this.accelerator);
		} else if(this.movingDown) {
			if(this.velocityY <= 0.0) this.velocityY = 1.0;
			this.velocityY = Math.min(200.0,this.velocityY * this.accelerator);
		} else if(this.velocityY <= -0.01 || this.velocityY >= 0.01) this.velocityY = this.velocityY * this.deccelerator; else this.velocityY = 0.0;
		if(this.movingLeft) {
			if(this.velocityX >= 0.0) this.velocityX = -1;
			this.velocityX = Math.max(-200.,this.velocityX * this.accelerator);
	...
	,howManyAroundNot: function(x,y,target) {
		var count = 0;
		if(this.testPosition(x,y)) {
			var _g = -1;
			while(_g < 2) {
...
flash.events.TimerEvent.__name__ = true;
...
flash.text._AntiAliasType.AntiAliasType_Impl_.__name__ = true;
...
flash.text.TextField.__name__ = true;
	...
	,__renderDOM: function(renderSession) {
		...
		if(this.__dirty) {
			if(this.__text != "") {
				...
				var style1 = this.__div.style;
				style1.setProperty("font",this.__getFont(this.__textFormat),null);
				style1.setProperty("color","#" + StringTools.hex(this.__textFormat.color,6),null);
				style1.setProperty("width",this.__width + "px",null);
				style1.setProperty("height",this.__height + "px",null);
				var _g = this.__textFormat.align;
				switch(_g[1]) {
				case 3:
					style1.setProperty("text-align","center",null);
					break;
				case 1:
					style1.setProperty("text-align","right",null);
					break;
				default:
					style1.setProperty("text-align","left",null);
				}
				this.__div.innerHTML = this.__text;
				style1.setProperty("opacity",Std.string(this.__worldAlpha),null);
				style1.setProperty(renderSession.transformProperty,this.__worldTransform.to3DString(renderSession.z++),null);
...
flash.text.TextFormatRange.__name__ = true;
...
flash.text.TextFormat.__name__ = true;
...
flash.utils.Timer.__name__ = true;
